        ...
        ? roles.filter(
            (item) =>
                requiredRoles.findIndex(
                    (required) => required === item.id || required.id === item.id
                ) !== -1
        )
        : []
}
...
const checkInject = function (el, binding, vnode) {
    ...
    const $auth = binding.instance.$auth
    ...
    install: (app) => {
        ...
        app.mixin({
            ...
            methods: {
                ...
                $actionCheck(check, type) {
                    const fix = this.actionFix || this.name
                    return this.$auth(fix + '.' + check, type)
                },
            },
        })
    },
