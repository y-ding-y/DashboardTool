<template>
    <div>
        <a-dropdown class="header-item">
            <div>
                <a-tooltip placement="left" :title="$t(isFavored ? 'cancle' : 'collect')">

                    <component @click="checkFavor" :is="isFavored ? 'heart-filled' : 'heart-outlined'"
                        :class="isFavored ? 'favor' : null"></component>
                </a-tooltip>
            </div>
                ...
                <a-menu :selected-keys="[$route.path]">
                    <a-menu-item v-for="item in favorList" :key="item.url">
                        <a :href="item.url" :target="item.type == 'link' ? '_blank' : '_self'">
                            <component :is="item.type+'-outlined'"></component>
                            <span v-html="favorName(item)"></span>
                        </a>
                    </a-menu-item>
                </a-menu>
            </template>
