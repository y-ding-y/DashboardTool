    ...
    <div class="search-area" ref="root">
        ...
        <div class="search-item" :key="index" v-for="(col, index) in searchCols">
            <div v-if="col.dataType === 'boolean'" :class="['title', { active: col.search.value !== undefined }]">
                ...
                <a-switch @change="onSwitchChange(col)" class="switch" v-model:value="col.search.value" size="small"
            ...
            <div v-else-if="col.dataType === 'time'" :class="['title', { active: col.search.value }]">
                <template v-if="col.title"> {{ col.title }}: </template>
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-time-picker :format="col.search.format" v-model:value="col.search.value" placeholder="选择时间"
            ...
            <div v-else-if="col.dataType === 'date'" :class="['title', { active: col.search.value }]">
                <template v-if="col.title"> {{ col.title }}: </template>
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-date-picker :format="col.search.format" v-model:value="col.search.value" @change="onDateChange(col)"
            ...
            <div v-else-if="col.dataType === 'datetime'" class="title datetime active">
                <template v-if="col.title"> {{ col.title }}: </template>
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <a-date-picker :format="col.search.format" v-model:value="col.search.value"
                    @change="(date, dateStr) => onCalendarChange(date, dateStr, col)"
                    @openChange="(open) => onCalendarOpenChange(open, col)" class="datetime-picker" size="small"
                    show-time :getCalendarContainer="() => $refs.root" />
            </div>
            <div v-else-if="col.dataType === 'select'" :class="['title', { active: col.search.value !== undefined }]">
                <template v-if="col.title"> {{ col.title }}: </template>
                <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
                <template v-slot:content>
                    <a-select :allowClear="true" :options="col.search.selectOptions" v-model:value="col.search.value"
                        placeholder="请选择..." @change="onSelectChange(col)" class="select" size="small"
                        :get-popup-container="() => $refs.selectRoot">
                    </a-select>
                </template>
            </div>
            <div v-else :class="['title', { active: col.search.value }]">
                <a-popover @visibleChange="onVisibleChange(col, index)" v-model:value="col.search.visible"
                    placement="bottom" :trigger="['click']" :get-popup-container="() => $refs.root">
                    ...
                    <template v-slot:title>
                        <div class="search-overlay">
                            <a-input :id="`${searchIdPrefix}${index}`" :allow-clear="true" @keyup.esc="onCancel(col)"
                                @keyup.enter="onConfirm(col)" v-model:value="col.search.value" size="small" />
                        </div>
                    </template>
                </a-popover>
            ...
            searchCols(newVal, oldVal) {
                if (newVal.length != oldVal.length) {
                    ...
                    if (!fastEqual(newConditions, this.conditions)) {
                        ...
                        $emit(this, 'change', this.conditions, this.searchOptions)
                    }
                }
            },
        },
